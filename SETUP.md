...
5. Generate the video catalog:
   ```bash
   php refreshVideos.php
   ```
   This writes `videos.json` to the site root. Re-run this any time you add
   videos to Vimeo.
...
- `processPhotos.php` and `refreshVideos.php` are CLI-only — they check
  `php_sapi_name()` and return HTTP 403 if accessed via a browser. No special
  server config needed to protect them, but blocking at the web server level
  is also fine.
...
├── refreshVideos.php
├── suggestions.php
├── videos.json                     ← generated by refreshVideos.php
├── composer.json
